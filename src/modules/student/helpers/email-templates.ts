import { DeviceInfo } from "src/common/services/device-detection.service";

// ================================================================
// 1. HELPER: Parse User-Agent into readable English details
// (Optional – you can remove this if using DeviceDetectionService)
// ================================================================
...
// ================================================================
// 2. MAIN BUILDER: Login Alert Email
// ================================================================
export function buildLoginAlertEmail(
  ...
  const content = `
    ...
    <div style="background: #f8fafc; border-radius: 12px; padding: 20px 24px; margin: 24px 0; border: 1px solid #e2e8f0;">
      <table width="100%" cellpadding="8" cellspacing="0" style="border-collapse: collapse;">
        <tr>
          ...
          <td style="padding: 8px 0; color: #0f172a;">${ipAddress || 'Unavailable'}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td style="padding: 8px 0; color: #0f172a;">${deviceInfo.deviceName}</td>
        </tr>
        <tr>
        ...
        <tr>
          <td style="padding: 8px 8px 8px 0; font-weight: 600; color: #475569;">📱 Type</td>
          <td style="padding: 8px 0; color: #0f172a;">${deviceInfo.deviceType}</td>
        </tr>
      </table>
    </div>

    ${paragraph(`
      <div style="background: #fef2f2; border-right: 4px solid #ef4444; padding: 12px 16px; border-radius: 6px; color: #991b1b; font-size: 14px;">
    ...
    <div style="margin-top: 28px; text-align: center;">
      <a href="#" 
         style="background: #ffffff; color: #334155; padding: 12px 24px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: 600; font-size: 15px; border: 1px solid #cbd5e1; margin: 4px; min-width: 140px;">
         Yes, this was me
      </a>
      <a href="https://example.com/reset-password" 
         style="background: #dc2626; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 8px; display: inline-block; font-weight: 700; font-size: 15px; box-shadow: 0 4px 12px rgba(220, 38, 38, 0.3); margin: 4px; min-width: 140px; border: 1px solid #dc2626;">
...
export function buildApplicationApprovedEmail(
  ...
  const content = `
    ...
    <div style="background: #f0fdf4; border-radius: 12px; padding: 16px 20px; margin: 20px 0; border: 1px solid #bbf7d0; border-right: 4px solid #22c55e;">
...
export function buildApplicationReceivedEmail(
  ...
  const content = `
    ...
    <div style="background: #fffbeb; border-radius: 12px; padding: 16px 20px; margin: 20px 0; border: 1px solid #fde68a; border-right: 4px solid #f59e0b;">
...
export function buildApplicationRejectedEmail(
  ...
  const content = `
    ...
    <div style="background: #fef2f2; border-radius: 12px; padding: 16px 20px; margin: 20px 0; border: 1px solid #fecaca; border-right: 4px solid #dc2626;">
    ...
    ${paragraph(`
      <div style="font-size: 14px; color: #64748b; border-top: 1px solid #e2e8f0; padding-top: 16px; margin-top: 8px; background: #f8fafc; border-radius: 8px; padding: 12px 16px;">
...
export function buildPasswordResetEmail(
  ...
  const content = `
    ...
    <div style="background: #EFF6FF; border-radius: 12px; padding: 24px 20px; margin: 24px 0; border: 2px dashed #1d4ed8; text-align: center;">
      <span style="display: block; font-size: 42px; font-weight: 800; letter-spacing: 8px; color: #0f172a; font-family: 'Courier New', monospace; background: #ffffff; padding: 12px 20px; border-radius: 8px; border: 1px solid #e2e8f0; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        ${code}
      </span>
      <p style="margin: 12px 0 0 0; font-size: 13px; color: #1d4ed8;">
        Enter this code on the code verification page.
      </p>
    </div>

    ${paragraph(`
      <div style="background: #fef2f2; padding: 12px 16px; border-radius: 6px; color: #991b1b; font-size: 14px; border: 1px solid #fecaca; border-right: 4px solid #dc2626;">
